...
public class GuiGasCentrifuge extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		if (container.getTile() != null) {
			GuiUtil.guiEnergyMeter(container.getTile().getEnergy(), guiLeft + 153, guiTop + 6);
		}
	}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		String s = container.getTile().hasCustomName() ? container.getTile().getName() : I18n.format(container.getTile().getName(), new Object[0]);
		fontRendererObj.drawString(s,
				xSize / 2 - fontRendererObj.getStringWidth(s) / 2, 10,
				4210752);

		AIsotope<?,?> iso = container.getTile().getDesiredIsotope();
		...
		String prodAssay = container.getTile().getProductAssay()/100d + "%";
		fontRendererObj.drawString(prodAssay,
				69 - fontRendererObj.getStringWidth(prodAssay) / 2, 49,
				4210752);

		String tailAssay = container.getTile().getTailsAssay()/100d + "%";
		...
		if (mouseX>guiLeft + 154 && mouseX<guiLeft + 170 && mouseY>guiTop + 7 && mouseY<guiTop + 78) {
			drawHoveringText(Arrays.asList(new String[]{container.getTile().getEnergy().getStored() + "/" + container.getTile().getEnergy().getCapacity() + " " + NuclearSciences.instance.config.energyUnit}), mouseX-guiLeft, mouseY-guiTop);
		}
	}
